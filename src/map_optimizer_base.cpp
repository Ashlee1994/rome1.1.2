...
#undef ELT
    ...
    void HiddenVariableMonitor::monitorHiddenVariableChanges(const MetaDataTable& metadata_old,int first_image,const MetaDataElem* metadata_new,int nr_images)
    {
        for (int iimage = 0; iimage < nr_images; iimage++)
        {
            ...
            // Some orientational distance....// TOOD calculateAngularDistance....
            sum_changes_optimal_orientations += (new_rot-old_rot)*(new_rot-old_rot) + (new_tilt-old_tilt)*(new_tilt-old_tilt) + (new_psi-old_psi)*(new_psi-old_psi);
...
#undef COUTMETADATA
    ...
    //
    std::pair<std::string, std::string> readFromOptimizer(std::string fn_optimiser)
    {
...
#define CINMETADATADOUBLE(V) optimiserFile >> line;optimiserFile >> doubleTmp;NODE0ONLY std::cout<<std::setw(30)<<line<<" "<<doubleTmp<<std::endl;
#define CINMETADATASTR(V) optimiserFile >> line;optimiserFile >> stringTmp;NODE0ONLY std::cout<<std::setw(30)<<line<<" "<<stringTmp<<std::endl;
                    //
...
#undef CINMETADATASTR
                    ...
                    NODE0ONLY std::cout<<line<<std::endl;
    ...
    void transferMetaData(MetaDataTable& metadata)
    {
        //
...
#define METADATA_LEN 11
        FDOUBLE* metadataSendBuf = (FDOUBLE*)_mm_malloc(sizeof(FDOUBLE)*nr_local_images*METADATA_LEN,64);
        ...
        NODE0ONLY metadataRecvBuf = (FDOUBLE*)_mm_malloc(sizeof(FDOUBLE)*nr_global_images*METADATA_LEN,64);
        for	(int iimage = 0; iimage < nr_local_images; iimage++)
        {
#define ELT(I,T) metadataSendBuf[iimage*METADATA_LEN+I] = metadata[iimage+first_local_image].T;
            METADATA_ELEM
#undef ELT
        }
        for	(int rank = 0; rank < nodes; rank++){
            ...
            nr_local_images = divide_equally_pool(nr_global_images, nr_pool, nodes, rank, first_local_image, last_local_image);
#else
            nr_local_images = divide_equally(nr_global_images, nodes, rank, first_local_image, last_local_image);
#endif
            if (rank == 0){
        ...
        MPI::COMM_WORLD.Gatherv(metadataSendBuf, nr_local_images, oneMetadataElem, metadataRecvBuf, rcounts, displs, oneMetadataElem, 0);
        
        NODE0ONLY{
            for (int iimage = 0;iimage < nr_global_images;iimage++){
                // set the metadata
#define ELT(I,T) metadata[iimage].T = metadataRecvBuf[iimage*METADATA_LEN+I];
                METADATA_ELEM
#undef ELT
            }
        }
        oneMetadataElem.Free();
        _mm_free(metadataSendBuf);
        NODE0ONLY _mm_free(metadataRecvBuf);
#undef METADATA_ELEM
#undef METADATA_LEN
    }
#endif
