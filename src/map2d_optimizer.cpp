...
void prepare()
{
    ...
    for (int igroup = 0; igroup < mlModel.nr_groups; igroup++)
        MPI::COMM_WORLD.Bcast(mlModel.sigma2_noise[igroup].wptrAll(),mlModel.ori_Fsize,MPI::DOUBLE,0);
    for (int iclass = 0; iclass < nr_classes; iclass++){
        MPI::COMM_WORLD.Bcast(mapModel.Irefs[iclass].wptr(),mapModel.Irefs[iclass].dimzyx,MPI::DOUBLE,0);
        MPI::COMM_WORLD.Bcast(mlModel.tau2_class[iclass].wptrAll(),mlModel.ori_Fsize,MPI::DOUBLE,0);
        MPI::COMM_WORLD.Bcast(mlModel.data_vs_prior_class[iclass].wptrAll(),mlModel.ori_Fsize,MPI::DOUBLE,0);
    }
#endif
    Mavg.fini();
}
...
void prepareExpData()
{
    ...
    exp_Mweight             	= (double*)_mm_malloc(sizeof(double)*nr_images*nr_classes*sampler2d.NrPoints(adaptive_oversampling),64);
...
void expectationSomeParticles()
{
	...
	for (exp_ipass = 0; exp_ipass < nr_sampling_passes; exp_ipass++)
	{
        ...
        samplingGrid.computeGrid(sampler2d, exp_current_oversampling);
    ...
    for (int iclass = exp_iclass_min; iclass <= exp_iclass_max; iclass++)
    {
        for (int ipsi = 0; ipsi < exp_nr_psi; ipsi++)
        {
            for (int iover_rot = 0; iover_rot < exp_nr_over_rot; iover_rot++)
            {
                ...
                samplingGrid.getProjectMatrix(A, 0, ipsi, iover_rot);
        ...
        for (int iclass = exp_iclass_min; iclass <= exp_iclass_max; iclass++){
            for (int ipsi = 0; ipsi < exp_nr_psi; ipsi++){
                for (int iover_rot = 0; iover_rot < exp_nr_over_rot; iover_rot++)
                {
                    ...
                    global_data_stream.foutDouble(samplingGrid.exp_over_rot[ipsi][iover_rot], "getAllSquaredDifferences()_exp_over_rot[iorient][iover_rot]", __FILE__, __LINE__);
                    global_data_stream.foutDouble(samplingGrid.exp_over_tilt[ipsi][iover_rot], "getAllSquaredDifferences()_exp_over_tilt[iorient][iover_rot]", __FILE__, __LINE__);
                	...
                	for (int iover_rot = 0; iover_rot < exp_nr_over_rot; iover_rot++)
                	{
                        for (int iover_trans = 0; iover_trans < exp_nr_over_trans; iover_trans++)
                        {
                            ...
                            else
                            {
                                ...
                                auto pdf_offset = mlModel.calculatePdfOffset(offset_x,offset_y,mlModel.prior_offsetx_class.rptrAll()[iclass],
...
#pragma omp parallel for
    for (int iimage = 0;iimage < exp_nr_images; iimage++)
    {
        double* sorted_weight = (double*)_mm_malloc(sizeof(double)*exp_Mweight_xsize,64);
