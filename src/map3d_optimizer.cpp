...
void setupMLoptimizer()
{
    ...
    // ------------ initialize sampling ---------- //
    // set sampling 3d
    sampler3d.initialize(offset_step,offset_range,-1,sampler3d_healpix_order, sampler3d_fn_sym);
...
void prepare()
{
    ...
    mlModel.calculateSumOfPowerSpectraAndAverageImage(Mavg, images, do_zero_mask, metadata, mapModel);
    ...
    if (iter==0) {
        ...
        int* local_temp2 = (int*)_mm_malloc(sizeof(int)*mlModel.nr_groups,64);
...
void iterate()
{
    ...
    mapModel.updateCurrentResolution(mlModel.data_vs_prior_class,set_by_ini_high);
    ...
    for (iter = iter + 1; iter <= nr_iter; iter++)
    {
        ...
        // NOTE : each thread has its own nr_classes 3D map,then reduce all 3D map to one.
        // it is a not good solution.. // TODO
        // for 600*600*600 3D map,nr_classes=10,nr_threads = 64,memory space : 1029.96826171875 GB.
        mapModel.reduceThreadMap();
        ...
        mapModel.reduceData(false);
#else
        mapModel.reduceData(true);
#endif
        ...
        bool set_by_ini_high = ini_high > 0. && (iter == 0 || (iter == 1 && do_firstiter_cc) );
        mapModel.updateCurrentResolution(mlModel.data_vs_prior_class,set_by_ini_high);
        NODE0ONLY mapModel.printResolution(mlModel.data_vs_prior_class,set_by_ini_high);
...
void prepareExpData()
{
    ...
    //
    particleModel.setup(nr_images, current_size, coarse_size, exp_nr_all_trans);
...
void expectation()
{
    ...
    while (nr_images_done < nr_local_images)
    {
        ...
        hiddenVarMonitor.monitorHiddenVariableChanges(metadata, my_first_image, &exp_metadata[0], exp_nr_images);
...
void expectationSomeParticles()
{
    ...
    for (exp_ipass = 0; exp_ipass < nr_sampling_passes; exp_ipass++)
    {
        ...
        samplingGrid.computeGrid(sampler3d, exp_current_oversampling);
        ...
        particleModel.preShiftedImagesCtfsAndInvSigma2s(exp_Fimgs_shifted_real, exp_Fimgs_shifted_imag,
                                                        exp_local_Minvsigma2s,
                                                        exp_local_sqrtXi2, do_cross_correlation,
                                                        exp_local_Fctfs, exp_current_size,
...
namespace CoarseAndFineSearch
{
    ...
    //
    inline void get_projected_class(int iclass,int idir,int ipsi,int iover_rot,
                                    FDOUBLE* &Frefctf_real,FDOUBLE* &Frefctf_imag,int n_start,int n_end)
    {
        FDOUBLE A[3][3];
        samplingGrid.getProjectMatrix(A, idir, ipsi, iover_rot);
                ...
                for (int idir = 0; idir < exp_nr_dir; idir++){
                    ...
                    for (int ipsi = 0; ipsi < exp_nr_psi; ipsi++){
                        ...
                        for (int iover_rot = 0; iover_rot < exp_nr_over_rot; iover_rot++)
                        {
                            ...
                            // get projected image
                            get_projected_class(iclass,idir,ipsi,iover_rot,Frefctf_real,Frefctf_imag,0,exp_current_Fsize2);
                            ...
                            global_data_stream.foutDouble(samplingGrid.exp_over_rot[iorient][iover_rot], "getAllSquaredDifferences()_exp_over_rot[iorient][iover_rot]", __FILE__, __LINE__);
                            global_data_stream.foutDouble(samplingGrid.exp_over_tilt[iorient][iover_rot], "getAllSquaredDifferences()_exp_over_tilt[iorient][iover_rot]", __FILE__, __LINE__);
                            global_data_stream.foutDouble(samplingGrid.exp_over_psi[iorient][iover_rot], "getAllSquaredDifferences()_exp_over_psi[iorient][iover_rot]", __FILE__, __LINE__);
            ...
            for (int iclass = exp_iclass_min; iclass <= exp_iclass_max; iclass++)
            {
                for (int idir = 0; idir < exp_nr_dir; idir++)
                {
                    for (int ipsi = 0; ipsi < exp_nr_psi; ipsi++)
                    {
                        ...
                        FDOUBLE pdf_offset = (std::numeric_limits<FDOUBLE>::max)();
...
#pragma omp parallel for
    for (int iimage = 0; iimage < exp_nr_images; iimage++)
    {
        ...
        exp_metadata[iimage].ROT = samplingGrid.exp_over_rot[iorient][iover_rot];
        exp_metadata[iimage].TILT = samplingGrid.exp_over_tilt[iorient][iover_rot];
...
#pragma omp parallel for collapse(4) schedule(dynamic)
        ...
        global_data_stream.foutDouble(mapModel.thread_weight[0*nr_classes+0].wptr(), vol_size3, "backProjection()_backRef_weight_0", __FILE__, __LINE__);
        global_data_stream.foutDouble(mapModel.thread_weight[0*nr_classes+(nr_classes-1)].wptr(), vol_size3, "backProjection()_backRef_weight_N-1", __FILE__, __LINE__);
        global_data_stream.check();global_data_stream.flush();
#endif
    }
