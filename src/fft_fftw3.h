...
struct SOAComplexArray : public SOAComplexBase<T> {
    ...
    SOAComplexArray(size_t _length) : length(_length) {
        this->real = (T*)_mm_malloc(sizeof(T)*length,64);
        this->imag = (T*)_mm_malloc(sizeof(T)*length,64);
    }
    ...
    ~SOAComplexArray() {
        if(this->real) _mm_free(this->real); this->real = NULL;
        if(this->imag) _mm_free(this->imag); this->imag = NULL;
    }
};
