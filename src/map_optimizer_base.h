...
#define MAPOPTIMIZER_OLD_HIDDEN_VAR_ELT \
        ...
        ELT(double	, smallest_changes_optimal_offsets			, 9999999	)			SEP \
        ELT(double	, smallest_changes_optimal_orientations		, 9999999	)			SEP \
        ...
        ELT(double	, current_changes_optimal_offsets			, 0			)			SEP \
        ELT(double	, current_changes_optimal_orientations		, 0			)			SEP \
        ELT(double	, current_changes_optimal_classes			, 0			)			SEP \
...
#undef SEP
        ...
        void monitorHiddenVariableChanges(const MetaDataTable& metadata_old,int first_image,const MetaDataElem* metadata_new,int nr_images);
        ...
        // void calculateExpectedAngularErrors(int my_first_ori_particle,int my_last_ori_particle);
    };
    //
    ...
    public:
        ...
        void init(int _size){
            ...
            assert(!first);first 	= (int*)	_mm_malloc(sizeof(int)		*total_size,64);
            assert(!second);second 	= (double*)	_mm_malloc(sizeof(double)	*total_size,64);
        }
        void fini(){
            total_size = current_size = 0;
            if (first) _mm_free(first);if (second) _mm_free(second);
        }
