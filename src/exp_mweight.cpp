...
#else
	s = "{";
	s+= "iimage:"     + std::to_string(mui.iimage())
		+ ", iclass:" + std::to_string(mui.iclass())
		+ ", idir:"	  + std::to_string(mui.idir  ())
		+ ", ipsi:"	  + std::to_string(mui.ipsi  ())
		+ "}";
#endif
	return s;
}
...
void Exp_Mcoarse_Rot_significant_base::init(int _nr_classes,int _nr_dir,int _nr_psi,int _nr_images,int _nr_trans){
...
void Exp_Mcoarse_Rot_significant_full::init(int _nr_classes,int _nr_dir,int _nr_psi,int _nr_images,int _nr_trans){
    Exp_Mcoarse_Rot_significant_base::init(_nr_classes, _nr_dir, _nr_psi, _nr_images, _nr_trans);
    exp_Mcoarse_significant_full.init(nr_classes*nr_dir*nr_psi, nr_images*nr_trans);
...
void Exp_Mcoarse_Rot_significant_full::assignMcoarseSignificant(int iclass,int idir,int ipsi,std::vector<int>& iimage_trans,char value)
{
    for (auto &i : iimage_trans) exp_Mcoarse_significant_full[(iclass*nr_dir + idir)*nr_psi + ipsi].wptrAll()[i] = value;
}
...
void Exp_Mcoarse_Rot_significant_sparse::init(int _nr_classes,int _nr_dir,int _nr_psi,int _nr_images,int _nr_trans){
    Exp_Mcoarse_Rot_significant_base::init(_nr_classes, _nr_dir, _nr_psi, _nr_images, _nr_trans);
    {/*do nothing*/}
}
...
void Exp_Mcoarse_Rot_significant_sparse::allocMcoarseSignificant(int iclass,int idir,int ipsi)
{
    ...
    char* significant_images_trans = (char*)aMalloc(nr_images*nr_trans, 64);
    auto newit = exp_Mcoarse_significant_sparse.insert(it, std::make_pair(orientation,significant_images_trans));
    for (int i = 0; i < nr_images*nr_trans; i++)
...
void Exp_Mcoarse_Rot_significant_sparse::assignMcoarseSignificant(int iclass,int idir,int ipsi,std::vector<int>& iimage_trans,char value)
{
    auto orientation = std::make_tuple(iclass,idir,ipsi);
    auto it = exp_Mcoarse_significant_sparse.find(orientation);
    ERROR_CHECK(it == exp_Mcoarse_significant_sparse.end(), "cound found exp_Mcoarse_significant.")
    for (auto& i : iimage_trans) it->second[i] = value;
}
...
Exp_Mcoarse_Rot_significant_new::~Exp_Mcoarse_Rot_significant_new(){}
void Exp_Mcoarse_Rot_significant_new::init(int _nr_classes,int _nr_dir,int _nr_psi,int _nr_images,int _nr_trans,int do_local_searching)
{
    ...
    if (use_full_data) {
        ...
        exp_Mcoarse_Rot_significant_full->init(_nr_classes, _nr_dir, _nr_psi, _nr_images, _nr_trans);
    }
    if (use_sparse_data) {
        exp_Mcoarse_Rot_significant_sparse = sNew(Exp_Mcoarse_Rot_significant_sparse);
        exp_Mcoarse_Rot_significant_sparse->init(_nr_classes, _nr_dir, _nr_psi, _nr_images, _nr_trans);
    }
}
...
void Exp_Mcoarse_Rot_significant_new::assignMcoarseSignificant(int iclass,int idir,int ipsi,std::vector<int>& iimage_trans,char value)
{
    assert(use_full_data || use_sparse_data);
    if (use_full_data) exp_Mcoarse_Rot_significant_full->assignMcoarseSignificant(iclass, idir, ipsi, iimage_trans, value);
    if (use_sparse_data) exp_Mcoarse_Rot_significant_sparse->assignMcoarseSignificant(iclass, idir, ipsi, iimage_trans, value);
}
// exp_Rot_significant
